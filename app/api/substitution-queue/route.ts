...
export async function GET(req: NextRequest) {
  ...
  const entries = (rawEntries as unknown as SubstitutionPoolEntry[])
    .filter((e) => !inTeamIds.has(e.playerId));
  const baseQueue = buildBaseQueue(entries);
  const total = baseQueue.length;
  const totalPages = Math.max(1, Math.ceil(total / PAGE_SIZE));

  // Wallet search: find player's page and position
  const searchWallet = sp.get("searchWallet") ?? "";
  if (searchWallet) {
    const idx = baseQueue.findIndex(e => (e.player?.wallet ?? "").toLowerCase() === searchWallet.toLowerCase().trim());
    const foundPage = idx >= 0 ? Math.ceil((idx + 1) / PAGE_SIZE) : null;
    return NextResponse.json({ walletSearchResult: { found: idx >= 0, position: idx >= 0 ? idx + 1 : null, page: foundPage, poolEntryId: idx >= 0 ? baseQueue[idx].id : null } });
  }
